...
- **Changes accumulate under `## [Unreleased]`, and nothing in CI checks the fold.** Every feature pull request writes its entry there, and the release folds it into the version heading as the play's step 6 says. `changelog-check.yml` asserts only that the version heading exists, so a release that added a fresh heading and left `[Unreleased]` standing above it would pass CI, and `publish.yml`, which slices the Release notes from the version heading down, would ship notes without those entries. Renaming the heading, rather than adding one, is what keeps them.
...
- **Downstream consumers pin a caret range.** `pipelex-mcp`, `pipelex-app`, `pipelex-starter-js` and `pipelex-method-apps` (in `webapp-js/package.json`) carry `@pipelex/sdk` as `^X.Y.Z`, and npm's caret does not cross a minor on a `0.x` version, so a minor release reaches them only when someone moves the range. `ledger/ledger.toml` declares no `release_followups` for `pipelex-sdk-js`, so nothing arms those bumps: file them yourself alongside the release item.
