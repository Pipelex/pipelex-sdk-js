...
### Changed — API base URL env var renamed `PIPELEX_API_URL` → `PIPELEX_BASE_URL` (breaking)

The environment variable the client reads for the API base URL is renamed from `PIPELEX_API_URL` to `PIPELEX_BASE_URL`, for consistency with the internal `baseUrl` naming and coordinated with the `MTHDS_API_URL` → `MTHDS_BASE_URL` rename in the `mthds` clients. There is no read alias — update any environment or deployment that sets `PIPELEX_API_URL`.
