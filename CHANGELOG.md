...
- **The `.mthds` PostToolUse hook bundle (`npm run build:hook` → `dist-hooks/check.mjs`).** New `src/hooks/` sources build a single dependency-free ESM file that agent plugins (first consumer: `pipelex-plugins`) vendor as a static hook asset: local lint/format via `@pipelex/tools-wasm` (offline, no credentials, format writes back in place), then the full bundle verdict from `POST /v1/validate` via this SDK (`allow_signatures: true`, server-rendered Markdown forwarded verbatim as the block reason). Engine-agnostic decision core (`check-core.ts`: per-stage verdict shapes → block / additionalContext / pass, mirroring the plxt-era hook's Stage-3 model), recursive parent-dir bundle gathering with caps (`bundle-gather.ts` — on overflow the validate stage reads as unavailable rather than risking a false block on an under-supplied bundle), and a fail-open posture throughout: no `PIPELEX_API_KEY`, network faults, timeouts, or any non-2xx skip the validate stage silently while the local lint/format verdicts still apply. `@pipelex/tools-wasm` is a normal npm devDependency (its published types replace the interim local shim); to bundle an unreleased engine build, point `PIPELEX_TOOLS_WASM_PATH` at a `vscode-pipelex/js/tools-wasm` checkout.
