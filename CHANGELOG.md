...
- **`resolve()` and `codegen()` on `PipelexApiClient` — the crate routes.** `client.resolve({ files })` returns the **normalized library crate** (`POST /v1/resolve`): the MTHDS standard's Library Crate Format, fully qualified refs, refinement flattened, natives materialized, with `fingerprint` and `mthds_version` riding **inside** the crate payload. `client.codegen({ files, kind: "types", target: "ts-zod" })` projects that crate into **stamped typed artifacts** plus their `codegen.lock` (`POST /v1/codegen`), through two explicit axes — `kind` (`types`) × `target` (`ts-zod` / `python-pydantic` / `python-structures`). Writing every artifact at its `path` and the `lock` content as `lock_filename`, both verbatim, reproduces a local `pipelex codegen types` run byte-for-byte, so the offline `pipelex codegen check` passes on the written tree; the SDK stays transport-only and does not write files. Both are Pipelex API extensions (not `x-mthds-protocol`) and follow the build routes' verdict discipline: a produced verdict is a **200** discriminated on `is_valid`, with the shared `CrateInvalidReport` as the invalid arm, while a no-verdict condition throws the typed `ApiResponseError` (request shape — including a `pipe_ref` on the concept-set-wide `types` kind — is a `422`; the reserved `method_ref` is a `501`). New exported types: `CrateRequestBase`, `ResolveRequest`, `ResolveValidReport`, `ResolveResponse`, `CodegenKind`, `CodegenTarget`, `CodegenRequest`, `CodegenValidReport`, `CodegenResponse`. **Note:** both routes are served by any `pipelex-api` runner; hosted exposure is partial. `api-dev.pipelex.com` (`pipelex-hosted@0.9.0`) serves them with the verdict discipline above intact, while `api.pipelex.com` (`pipelex-hosted@0.2.6`) still answers a gateway `403` pending the deploy. `lint` / `format` remain `403` on both origins — only the crate routes were allowlisted — so point `baseUrl` at a runner for those. See [`docs/crate-routes.md`](./docs/crate-routes.md).

### Changed

- **`make test-e2e` now preflights the server, honors `.env`, and fails in one line.** With nothing listening on the target (default `http://localhost:8081`), the target used to hand the work straight to Vitest, where all three e2e suites failed in their `beforeAll` reachability check and each printed a serialized `ECONNREFUSED` dump — burying the actionable sentence under socket stacks. The target now runs one `curl` first and, when nothing answers, prints the unreachable URL plus how to start a runner, then exits non-zero. The per-suite `beforeAll` guard is unchanged in spirit and still covers a direct `npm run test:e2e`.

  Two fixes ride along. **The reachability probe moved from the origin-level `/health` to `/v1/version`** (in the preflight and in all three suites' `beforeAll`): only a bare runner serves `/health`, and a hosted origin 404s it while serving `/v1/health` — so every hosted target was reported as "no server reachable" when it was in fact up. `/v1/version` is the one unauthenticated route both answer. And **`PIPELEX_E2E_BASE_URL` / `PIPELEX_API_KEY` are now read from `.env`** when the shell does not already supply them, resolved once in the Makefile and exported, so the URL the preflight checks is the URL the suites call. Precedence is the dotenv convention: shell environment (or a `make test-e2e PIPELEX_E2E_BASE_URL=...` override) first, then `.env`, then the local default; the resolved target is echoed before the run, since `.env` can now supply it invisibly. The target is stripped of trailing slashes before the probe is built, the way the client normalizes `baseUrl`, so a value ending in `/` cannot make the preflight request `//v1/version` — which a runner does not route — and report a live server as unreachable. `PIPELEX_E2E_BASE_URL` stays distinct from the client's own `PIPELEX_BASE_URL` on purpose — sharing it would let an unset variable fall through to the client's hosted default and quietly aim a test run at the hosted API instead of failing locally.
...
- **Docs caught up with the v0.11.0 methods paging.** The README's product-routes example still read `const methods = await client.listMethods()`, as if an array came back, and the architecture doc's "Methods catalog" bullet still described read responses as `MethodData` with no mention of paging. Both now describe the `MethodPage` shape, the `MethodSummary` row, `iterateMethods` and its past-empty-pages rule, and `deletion_state` on list rows — matching the "Runs list/detail/update" bullet that already documented the same treatment for `listRuns`.
