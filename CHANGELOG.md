...
- **The validate report's typed valid arm now mirrors the wire field-for-field.** `PipelexValidationReport` gained three fields it had been letting ride through untyped on the protocol's extension index signature, so reaching them no longer needs a cast. `warnings` carries the advisory lints on a **valid** bundle — same item shape as `validation_errors[]`, so one parser serves both channels, but they never flip `is_valid`; it is where the new `hint_*` error types live. `liftable_pipes` lists the pipes the runtime may skip when an optional slot resolves absent, with the new `LiftablePipeEntry` type, which is build-time visibility for "this step may not run" rather than a verdict. Both have been on the wire since `pipelex` 0.38.0 and were simply never mirrored. `input_form` is new with `pipelex` 0.52.0: per-pipe input-form descriptors derived from authored facts rather than the emitted JSON Schema, so a renderer can build a fill-in form from the verdict alone.

  `input_form` is typed **optional** and its payload stays opaque, both deliberately. Opaque because `@pipelex/mthds-form` owns the typed descriptor vocabulary and has already absorbed the 0.52 reshape — a second copy here would be free to drift from the one the server emits. Optional because the standard makes it an opt-in structured view, absent from any verdict that did not ask for it — which `pipelex-api` 0.18.0 now enforces, after 0.17.0 briefly emitted it on every valid verdict. Optional reads a response from either runner correctly, so no consumer had to change when the gate landed.

- **`views`, the structured-view opt-in, on `validate` and `validateFiles`.** A sibling to `render`: `render` asks the server for *rendered text* and adds a `rendered_<format>` field, `views` asks for a *structured view* and adds a same-named top-level field — today only `input_form`. The two lists are independent, each resolving its own tokens against its own supported set, and unknown tokens are lenient-ignored on both rather than a `422`. Unlike `render`, which this client always populates with `markdown`, `views` is sent **only when a caller names a token** — an opt-in view exists so the default response stays byte-identical for the consumers that would discard it. `pipelex-api` resolves the token as of 0.18.0; against 0.17.0 it was inert (silently ignored, never a `422`), which is the lenient behavior that lets a client adopt a token before its server does.

- **`ValidationErrorItem` is a complete mirror at last.** It gained `missing_pipe_code` (symmetrical with the `missing_concept_code` already there) and `suggested_fix`, the server's deterministic repair proposal, along with the `SuggestedFix` / `FixOp` / `FixSafety` vocabulary it carries — a discriminated union over the seven patch kinds (`set_key`, `ensure_table`, `delete_key`, `delete_table`, `rename_table_key`, `move_key`, `remap_value`), so narrowing on `kind` reaches each op's own members without a cast. These are semantic patches over the `.mthds` document rather than a text diff, which is what lets an applier preserve the author's formatting. The gap was pre-existing and had been deferred because closing it properly meant porting the whole family; adding `warnings` made that same item type ride a fourth surface, which is what settled it.

- **`method_id` is a typed run option.** `execute`, `start`, and `startAndWaitForResult` now take the hosted platform's `method_id` as a first-class named option, through the new `PipelexRunOptions` / `PipelexStartOptions` types (the protocol's `RunOptions` / `StartOptions` intersected with the new `PipelexHostedRunExtensions`). It is a pure pass-through — the platform resolves the id against the org's catalog, and nothing is expanded client-side. Alone it is a run source; alongside an inline source the inline source is what runs and the id is recorded as run-history linkage on the Run row, which is what makes the run appear under `GET /v1/runs?method_id=`. It rides the blocking `execute` fallback too, so a bare runner answers the `422` that names it rather than the client silently dropping it. An empty string is treated as absent. Typing its own platform's arguments is what a hosted client is for; `extra` remains the escape hatch for extensions this client does not know about. The doctrine is normative across the layered client stack, not a preference of this SDK.
...
- **Breaking: every optional member of `ValidationErrorItem` widened from `T` to `T | null`.** The two channels serialize an unset locator differently and the SDK's single type has to be honest about both: the invalid arm and the crate routes drop the key (`exclude_none` server-side) while the valid arm — which is what carries `warnings[]` — does not, so the identical item arrives there with explicit `null`s. A truthiness check reads both and needs no change; an `=== undefined` check was already wrong on one of them and now fails to compile.
...
- **Documentation: no more citations a reader cannot open.** This package is public, and several docstrings, comments and doc passages cited internal specs and the conformance suite by bare repo-relative path — `docs/specs/pipelex-platform-api.md`, `docs/specs/pipelex-mthds-protocol.md`, `conformance/conformance/usage_records.py`. Those paths resolve to nothing for anyone who clones this repo, so they read as rot rather than as a deliberate boundary. Each site now states the rule it was citing instead: the layered extension policy behind `PipelexHostedRunExtensions` and the reserved-`extra` guard, and the MTHDS Protocol's ownership of the `TokensUsageRecord` wire contract. No behaviour changed.
