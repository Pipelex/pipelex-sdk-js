...
A run that ends without completing throws a `RunFailedError` from `waitForResult`, `startAndWaitForResult` and `downloadArtifacts`, and comes back as the `failed` arm of `getRunResult`. Its `status` is the run's terminal status and its `error` is the run's stored error report, typed whole as `RunErrorReport`: the reason in `message` (which names the failing pipe), `error_domain`, `type_uri` and `retryable` to branch on, `user_action` as the next step, and the inference details. It is `null` when the run ended without a report. The report is the runner's VERBOSE one, provider text included, so what a person sees is your presentation:
