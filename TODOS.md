...
- [ ] **Expose `/v1/lint` and `/v1/format` on the hosted origins** — cross-repo (`pipelex-server/infra` allowlist + `pipelex-server/platform` tooling proxy). **The crate half is done:** `resolve` and `codegen` are served on both `api-dev.pipelex.com` and `api.pipelex.com` (`pipelex-hosted@0.10.1`, measured 2026-08-23 with a real key), verdict discipline intact. Still open: **`lint` / `format`, which were left out of the allowlist change and still 403 on both origins** — exactly the "fix all four at once" trap the note warned about. Tracked in [`wip/hosted-exposure-crate-and-tools-routes.md`](./wip/hosted-exposure-crate-and-tools-routes.md).
- [x] **`ValidationErrorItem` was missing `missing_pipe_code` + `suggested_fix`** — closed while mirroring the `pipelex` 0.52 / `pipelex-api` 0.17 validation-report surface, which put the same item type on a fourth channel (the valid arm's advisory `warnings[]`) and forced the issue. The `SuggestedFix`/`FixOp`/`FixSafety` family is ported; see [`wip/validation-error-item-drift.md`](./wip/validation-error-item-drift.md) → "How it was closed". Still open there: whether `pipelex-sdk-python`'s mirror is partial in the same way.
