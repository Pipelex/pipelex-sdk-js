...
The lines are compared as the patch program received them. In a quoted heredoc (`<<'EOF'`), that is the script's text. In quotes, the shell removes the quotes a line closes and reopens, as `'\''` does to write an apostrophe in a single-quoted patch, and in double quotes or an unquoted heredoc it removes backslash escapes too, so `\"` in a double-quoted patch is compared as `"`. A line holding a `$` or backtick expansion is left out of the comparison, since the hook cannot know what it expanded to. When a line ends in other quoting than it began in, or in a backslash that joins it to the next line, the lines after it are read in quoting the hook does not follow, so none of that patch's lines is compared.
...
- **Another hook may run first.** A user's own `PostToolUse` hook that reformats `.mthds` files on `Bash` before this one would make the added lines differ from the file, and the hook would skip the file, with the note for a relative path and without one for an absolute path.
- **An unreached patch that only removes lines is checked.** An absolute path in such a patch, in a branch the script did not take or a function it never called, is checked, since the patch added nothing that could show it did not run.
- **A delete in another branch can hide the note.** When one branch of a script writes a relative path and another deletes it, and no file exists where the hook resolved the path, the hook takes the absence for the delete's doing and sends no note.
- **A header path holding an expansion is not read.** `*** Update File: $FILE` in an unquoted heredoc names no `.mthds` file as written, so the hook neither checks the file nor names it.
- **Every member of a pipeline is read as feeding its patch command.** In `apply_patch <<REAL | cat <<OTHER`, the text of `OTHER` is read as a patch the command applied, although a pipeline's data flows only to the right.
