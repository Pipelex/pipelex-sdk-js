...
Triage of the unresolved `chatgpt-codex-connector` and `cubic-dev-ai` review threads on [PR #31](https://github.com/Pipelex/pipelex-sdk-js/pull/31) (the `runCodegenCheck` offline drift check). Three documentation corrections were fixed on the branch, one finding was a false positive, item 1 was confirmed and deferred at first but has since landed upstream and been ported here on this branch (see its update below), and item 2 remains confirmed but deferred. Every PR thread has been answered and resolved, so this file is the only tracker the remaining deferral has. Items 3 onward were added later, by a full `/review` pass over the same branch; they have no PR thread and are introduced by their own heading below. A final section collects the follow-ups that belong to **`pipelex`** rather than to this SDK. **Status as of 2026-08-19: all five have landed upstream** on `pipelex`'s `feature/Codegen-followups`, committed but unpushed and unreleased — see [`upstream-codegen-followups-mirror.md`](upstream-codegen-followups-mirror.md) for what each one now requires of this repo, and in which direction it travels.

Both items were deferred on the same invariant, so it is worth stating once — it is still what holds item 2 back, and item 1's update below records why it stopped binding there. `src/codegen-check.ts` is a deliberate mirror of pipelex's `pipelex/codegen/`, and its module header (`src/codegen-check.ts:15-18`) makes that load-bearing: *"A verdict computed here must equal the one `pipelex codegen check` computes over the same bytes — including the drift detail strings, which are kept verbatim so a consumer switching between the CLI and this SDK reads the same report."* Where the SDK already matches the reference, tightening it unilaterally is not a fix — it makes a consumer's CI go red on `@pipelex/sdk` while the CLI calls the same tree current, which is the exact divergence the file already bends over backwards to avoid for CRLF (`normalizeNewlines`, `src/codegen-check.ts:626-637`).

For the record, the three items that *were* fixed on the branch: "Beside" → "Besides" in `README.md:11`; the overbroad "an incomplete list yields `isCurrent: true`" claim, corrected in both `docs/crate-routes.md:186` and its twin TSDoc bullet on `runCodegenCheck`; and the API sketch's stale `// byte-exact UTF-8 text` comment in `TODOS.md`, aligned with the shipped TSDoc. The false positive was `isWellFormedProjection` — pipelex's `_parse_projection` never checks `parts[2]` for emptiness, so the SDK's handling of the *reviewed* sub-question mirrors it and the proposed tightening would have been a regression. **Correction (2026-08-19):** the sentence that followed here originally read "mirrors it exactly", which overstated it. `_parse_projection` gates on two conditions, not one — `if len(parts) < 2` **and** `if kind is None or target is None`, resolving both axes against `CodegenKind`/`CodegenTarget`. The SDK omits the second gate, so `projection: BOGUS / NONSENSE` verifies here and is `hand-edited` upstream. That omission is deliberate and is now pinned in both directions by tests and named in `docs/crate-routes.md` under "Where it knowingly differs from the CLI" — but it is a real divergence, not a non-difference, and the reply on the cubic thread should not be read as covering it.

## 1. `parseStampFields` ignores uncommented lines inside the stamp block

**Reporters:** `chatgpt-codex-connector` (P1) and `cubic-dev-ai` (P1) — two independent threads on the same finding. **Location:** `parseStampFields`, `src/codegen-check.ts:191-210`.

**The finding, confirmed empirically.** Everything in this section describes the module *as it stood before the item-1 port*; the update below records how it was closed. Running the built module against the real vendored fixture with a bare `throw` injected between the markers:
...
yielded `isCurrent: true`, `drifts: []`. The trace was exactly as reported: `throw new Error("edited");` contains no `:`, so `separatorIndex === -1` (`src/codegen-check.ts:201`) skipped the line silently, and `body` is sliced from *below* the end marker so its hash was untouched and still matched both the stamp and the lock. That path is now unreachable — `parseStamped` rejects any header line without the comment prefix (`src/codegen-check.ts:116`) before `parseStampFields` is ever called.

The field-overwrite variant the reviews raised is **self-defeating rather than an amplifier**. Injecting `content_hash: 0000` did overwrite the field via `fields.set` (`src/codegen-check.ts:204`), but that makes the stamp disagree with the body and the file reports `hand-edited`. A colon-bearing but harmless line (`evil: for (;;) { break evil; }`) merely sets a junk key that nothing reads. So the only invisible variant is the one that changes nothing the hash covers.

**Why it was deferred: the reference did exactly the same thing.** `_parse_fields` in `../pipelex/pipelex/codegen/stamp.py`, as it stood at the time of the deferral:
...
That was a line-for-line twin of the TypeScript, including the `else raw_line.strip()` fallback that swallows an uncommented line. The same six mutations run through `parse_stamped` in the pipelex venv gave the identical verdict: the injected-`throw` case parsed fine, its recomputed body hash matched the stamp, and the reference reported **CURRENT**. Nothing in `check.py` compensated either — `_check_present_artifact` (`../pipelex/pipelex/codegen/check.py:118-130`) consults only `parsed.stamp.content_hash` and `locked_hash`. The SDK was at exact parity, so this was an upstream question, not an SDK one.

**Both sides have since moved.** `_parse_fields` (`../pipelex/pipelex/codegen/stamp.py:187-198`) now strips the prefix unconditionally — the tolerant fallback quoted above is gone — and carries a comment pointing at the gate in `parse_stamped` that makes the unconditional strip safe. This module mirrors that shape exactly; see the update below.
...
More decisively: there is no signature, no MAC, and no secret anywhere in the chain. An adversary who can write into the artifact can also recompute `content_hash` — this repo's own test helper `regenerate()` (`tests/helpers/codegen-stamp.ts:56-59`) does it in three lines — and edit the plaintext `codegen.lock` sitting beside it, yielding `isCurrent: true` over arbitrary content. Every variant, this one included, leaves an obvious git diff. The defence against malicious insertion is diff review, not the stamp. The realistic threat the check exists for is accidental edits and stale artifacts, and an accidental edit does not land inside a five-line `// key: value` block under a `DO NOT EDIT` banner.
...
**Update (2026-08-19): landed upstream, then ported here.** `parse_stamped` rejects any header line without the comment prefix and the `else raw_line.strip()` fallback is gone; `parseStamped` now carries the same gate and `parseStampFields` strips the prefix unconditionally.

The "ship upstream first" caution turned out not to bind, for the same reason recorded in [`upstream-codegen-followups-mirror.md`](upstream-codegen-followups-mirror.md): `runCodegenCheck` has never been published, so there is no released SDK to be stricter than the released CLI. The gate rejects only headers today's emitters cannot write, so it rides along with this PR.

Two things the port needed that the drafted patch did not anticipate, both load-bearing:

- **The gate must split on the wide boundary set, not `"\n"`.** The drafted patch used `headerRegion.split("\n")`. U+2028 and U+2029 terminate a `//` comment in ECMAScript, so under a narrow split a `.ts` header carrying a raw U+2028 followed by a statement is one prefixed line to the gate and two lines to the JavaScript engine — the check reports the file current while the engine runs the statement, since the header is not hashed. Reproduced by appending `\u2028globalThis.pwned = 1;` to the `crate_fingerprint` header line, the one field no reader validates: current under a narrow split, `hand-edited` under the wide one. Pinned by a parametrized row per boundary character, each of which goes red if either the gate or the wide split is removed.
- **The gate needs Python's `splitlines()` trailing rule, which `String.split` does not have.** `split` keeps a trailing empty segment when the text ends with a boundary; `splitlines` drops it. Harmless to a field loop that skips lines without a `:`, but the gate reads every segment — so a header whose last line merely *ends* with U+2028 would be rejected here and accepted by the CLI, the SDK-is-stricter direction this module exists to avoid. Hence `pythonSplitLines`. Confirmed against the reference: `parse_stamped` parses that file cleanly.

## 2. Drift ordering diverges from the reference CLI

**Reporter:** `cubic-dev-ai` (P2). **Location:** `sortedEntries` (`src/codegen-check.ts:647`), used by the locked-artifact loop at `:378` and the orphan loop at `:389`.
...
The existing path validation does **not** close this. `CONTROL_CHARACTER = /\p{C}/u` (`src/codegen-check.ts:573`) mirrors the same rejection in `validate_artifact_path` (`../pipelex/pipelex/codegen/lock.py:101`) exactly, and both engines agree on which categories it rejects: `Cc`, `Cf`, `Cs` (lone surrogate), `Co` (private use, U+E000–U+F8FF) and `Cn` (unassigned). A *paired* surrogate — a genuine astral character — is `Lo`/`So`, not `C`, and passes both sides. That is what narrows the window to U+F900–U+FFEF rather than all of U+E000–U+FFFF.

**Why it is unreachable in practice.** Codegen filenames are hardcoded constants, not derived from user text: `../pipelex/pipelex/codegen/emitters/python_pydantic.py:26` → `"models.py"`, `python_structures.py:34` → `"structures.py"`, `ts_zod.py:31-32` → `"types.ts"` and `"binder.ts"`. That is the entire universe of artifact paths — four flat ASCII names. Locked paths come from `build_lock` over those filenames (`emission.py:84`), and orphan paths must carry a codegen stamp, so they originate there too. Reaching the divergence needs a hand-edited `codegen.lock` — whose own header says *do not edit by hand* — carrying two exotically named entries in complementary Unicode ranges.

### 2b. The unreported divergence: full-string vs path-component ordering — since closed upstream

This one needed no Unicode at all. **It is resolved**: `_find_orphans` now sorts its result, so both reference loops use the plain full-string sort and this module already agrees with them. The analysis below is kept as the record of why it was filed; the update at the end of this item has the detail. As it stood, the reference was internally inconsistent between its two loops:

- **Locked-artifact loop** — `_check_locked_artifacts` (`../pipelex/pipelex/codegen/check.py:107`): `for path, locked_hash in sorted(lock.hash_by_path().items()):`, a plain `str` sort, i.e. code-point order over the whole path. This module's locked loop (`src/codegen-check.ts:378`) mirrors it correctly apart from 2a.
- **Orphan loop** — `_find_orphans` (`check.py:136`) iterated `_iter_stampable_files`, a pre-order DFS doing `sorted(directory.iterdir())` at each level (`check.py:154`). That was a path-*component* sort, not a full-string sort:
...
This module's orphan loop (`src/codegen-check.ts:389`) sorts by raw string, so for a tree holding a `models/` directory beside a `models.py`, the CLI reported `models/foo.py` first and this module the reverse. Nested stampable files are explicitly in scope — the `runCodegenCheck` TSDoc obliges the caller to "walk the whole tree, recursively", and `tests/codegen-check.test.ts:523-532` already exercises a nested file. One comparator could not mirror both loops — which is why upstream unified the two instead (`check.py:149`).

**Why both were deferred.** Only the **order** of the `drifts[]` array changes. `isCurrent`, every `category`, and every `detail` string are untouched, so the verdict and the reported text stay identical across implementations. `../docs/specs/pipelex-codegen.md:107-117` pins the algorithm but says nothing about ordering, so the promise being bent is an SDK-local doc line (`docs/crate-routes.md:139`, "each sorted by path — so a consumer can print it, snapshot it, or diff two runs"), not a spec contract. Roughly twenty lines of comparator plus tests, to fix an ordering difference that cannot redden anyone's CI, is the "guard impossible cases" churn the no-overengineering rule exists to prevent.

**Ready-to-apply change, if 2a is ever picked up.** Only the code-point comparator below is still wanted — the component-wise wrapper it originally came paired with was withdrawn when 2b closed upstream. There is no readable one-liner: `localeCompare` is locale-dependent, and spread or `Array.from` still compares single-character *strings* by code unit unless you call `.codePointAt(0)`.
...
verified to reproduce Python's `sorted()` exactly on the mixed corpus `["😀.py", "ﬀ.py", "豈.py", "𐀀.py", "a.py", "models.py", "models/foo.py"]`. It drops straight into `sortedEntries`, which stays one shared helper serving both call sites. The component-wise wrapper this paragraph originally called for — `left.split("/")` against `right.split("/")`, compared element-wise with `compareCodePoints`, to mirror the DFS — is no longer wanted: upstream unified its two loops on the full-string sort, so splitting `sortedEntries` in two would now *create* a divergence rather than close one.

**Testing.** The 2a case belongs in the existing `describe("runCodegenCheck — drift ordering")` block at `tests/codegen-check.test.ts:477`, whose style is `expect(report.drifts.map((drift) => [drift.path, drift.category])).toEqual([...])`. The nested-orphan case that this paragraph originally named as the one to pin first — `plus(tsFixture(), { path: "models/foo.ts", … }, { path: "models.ts", … })` with both stamped — is moot now that 2b is closed: both implementations already agree on it.

**Update (2026-08-19): 2b is closed upstream, with nothing to do here.** `_find_orphans` now sorts its result, so both reference loops use the plain full-string sort and `models/foo.py` vs `models.py` already agrees between the two implementations without touching this module — see U2 below. Only 2a survives, on the deferral grounds above, so the "both must move together" caution no longer applies and the component-wise wrapper sketched here is no longer wanted.

---

# Second review round (2026-08-19) — items 3 onward

Items 1 and 2 above came from the `chatgpt-codex-connector` and `cubic-dev-ai` threads on PR #31, all of which have since been answered and resolved. Everything from here down came from a later full `/review` pass over the same branch, has **no** PR thread, and was reproduced against the built `dist/` before being written down.
...
`validateCanonicalPath` (`src/codegen-check.ts:593`) rejects any path containing a backslash (the branch at `:597-599`), and `indexTreeFiles` (`src/codegen-check.ts:435`) applies it to **caller-supplied tree paths**, not only to lock paths. Reproduced: a tree file whose path uses a backslash separator throws `CodegenLockError`.
...
- **4a. Raw control characters are echoed into messages.** `raisePathError` (`src/codegen-check.ts:620`) interpolates the path verbatim — including on the branch that just rejected it *for containing control characters*. `describeError` (`src/codegen-check.ts:651`, called at `:480`) additionally embeds smol-toml's `TomlError.message`, which by construction contains a three-line codeblock of the raw lock source. A lock carrying an ANSI escape produces a message that still contains it, so terminal output can be rewritten or hidden.
...
- `checkPresentArtifact` (`src/codegen-check.ts:413`) comments that a branch is unreachable (`:418`), then folds it into a `hand-edited` drift (`:420-422`) rather than failing loudly. If the invariant ever breaks, every artifact of that type is silently reported hand-edited instead of surfacing the wiring bug. The reference's `comment_prefix_for` raises instead.
...
`save_text_to_path` was `path.write_text(text, encoding="utf-8")` (`pipelex/tools/misc/file_utils.py`, the write site now at `:86`). With the default `newline=None`, `write_text` translates every `\n` to `os.linesep`, so on Windows the emitted artifact was CRLF on disk — while `compute_content_hash` hashed the in-memory string, which is LF. The verdict still comes out right, because `load_text_from_path` reads back through universal-newline translation; that asymmetry is exactly what this branch discovered and why `runCodegenCheck` normalizes newlines rather than hashing raw bytes.
...
Cheap fix, and the one that was taken: pass `newline="\n"` at the write site so emitted artifacts are LF on every platform and the bytes match the hash that was recorded for them. Consumers already recommend a `.gitattributes` entry for the same tree, so this mostly removes a surprise rather than changing a contract.

**Landed upstream**, and fixed globally in `save_text_to_path` rather than only at the codegen write site, so every product text artifact is LF on every platform. The write is now `path.write_text(text, encoding="utf-8", newline="\n")` (`pipelex/tools/misc/file_utils.py:86`), with the reasoning above written into the function's own docstring (`:61-81`). **Invisible to this module**, which already normalizes newlines before hashing.
...
**A fourth, added 2026-08-19 while mirroring U3, and the most consequential of the four: the spec's "Lock format" section did not mention `lock_version` at all.** It enumerated the lock's contents as "each generated artifact's path and content hash, plus the source crate fingerprint and engine version" — an enumeration that went stale the moment upstream added the key, in the one section that frames the lock as a cross-language interchange format. A second implementer working from that prose builds a reader with no version gate, which is precisely the client the field exists to protect: strict enough to turn any added key into a hard no-verdict, with no way to say so actionably. The forward-compatibility policy is not derivable from the shape either — that an absent key means version 1, that the version is read *before* the key set is validated, and that a reader must learn a version before any writer emits it. **Written into the spec on 2026-08-19**; the conformance half is still open, since that section is skip-gated and de-skeletons only once a published pipelex ships the format.
